...
{
    public class VertexPainterUtils
    {
        ...
        public static GameObject GetSelection()
        {
            ...
            if (!curSelected)
            {
                UnityEditor.EditorUtility.DisplayDialog("Mesh Painter", "Please select an object in the scene!", "OK");
                return null;
            }
            return curSelected;
        }
        ...
        public static void SaveMesh(GameObject aGO)
        {
            ...
            if (!curMesh)
            {
                UnityEditor.EditorUtility.DisplayDialog("Mesh Painter", "Please select an object with a Mesh Attached!", "OK");
                return;
            }

            //Let the user select a path and name for this new mesh
            string filePath = UnityEditor.EditorUtility.SaveFilePanel("Save Mesh", Application.dataPath, curMesh.name + "_VersionNumber", "asset");
            string finalPath = "";
        ...
        public static void AddMeshCollider(GameObject aGO)
        {
            if (aGO)
            {
                ...
                if (foundMCollider)
                {
                    UnityEditor.EditorUtility.DisplayDialog("Mesh Painter", "There is already a Mesh Collider attached to this object", "OK");
                    return;
                }
        ...
        public static void HandleBrush(GameObject aGO, Mesh aMesh, Vector3 brushPos, BrushData brushData, VTXPainterState aState)
        {
            if (aGO && aMesh)
            {
                ...
                for (int i = 0; i < verts.Length; i++)
                {
                    ...
                    switch (aState)
                    {
                        case VTXPainterState.Sculpting:
                            verts[i] = SculptMesh(verts[i], normals[i], brushData, curFalloff);
                            break;

                        case VTXPainterState.Painting:
        ...
        public static Vector3 SculptMesh(Vector3 aVert, Vector3 aNormal, BrushData brushData, float aFalloff)
        {
            switch (brushData.sculptState)
            {
                case SculptState.NormalDir:
        ...
        public static Color PaintMesh(Color aColor, BrushData brushData, float aFalloff)
        {
