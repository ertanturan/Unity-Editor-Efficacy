...
namespace EditorUtility.Core
{
    public class VertexPainterWindow : EditorWindow
    {
        ...
        #region Variables
        public VTXPainterState m_CurrentState;
        ...
        public MouseData m_MouseData = new MouseData();
        public BrushData m_BrushData = new BrushData();
        ...
        {
            var win = GetWindow<VertexPainterWindow>(false, "Mesh Painter", true);
            win.m_CurrentState = VTXPainterState.None;
            win.GenerateGUIStyle();

            win.m_BrushData = new BrushData(1f, 0.5f, 0.5f, Color.white, SculptState.NormalDir, 1f, 1, 0f);
        }
        ...
        void OnGUI()
        {
            ...
            if (m_CurrentState != VTXPainterState.None)
            {
        ...
        void OnSceneGUI(SceneView sceneView)
        {
            ...
            if (m_CurrentState == VTXPainterState.Painting || m_CurrentState == VTXPainterState.Sculpting)
            {
                ...
                {
                    if (m_CurrentState == VTXPainterState.Painting)
                    {
                        Handles.color = m_BrushData.brushColor;
                    }

                    if (m_CurrentState == VTXPainterState.Sculpting)
                    {
                ...
                {
                    m_CurrentMesh = VertexPainterUtils.GetMesh(m_CurrentGO);
                }
            ...
            {
                VertexPainterUtils.HandleBrush(m_CurrentGO, m_CurrentMesh, m_CurHit.point, m_BrushData, m_CurrentState);
                if (m_CurrentMesh)
                {
                    UnityEditor.EditorUtility.SetDirty(m_CurrentMesh);
                }
                m_WasPainting = true;
            }
            else
        ...
        protected virtual void DrawToolUI()
        {
            ...
            {
                GameObject curSelected = VertexPainterUtils.GetSelection();
                Mesh curMesh = VertexPainterUtils.GetMesh(curSelected);

                VertexPainterUtils.FillMeshWithColor(curMesh, m_BrushData.brushColor);
            }
            ...
            if (GUILayout.Button("Add Mesh Collider", curSkin.button, GUILayout.Height(44), GUILayout.ExpandWidth(true)))
            {
                ...
                VertexPainterUtils.AddMeshCollider(curSelected);
            }
            ...
            if (GUILayout.Button("Save Mesh", curSkin.button, GUILayout.Height(44), GUILayout.ExpandWidth(true)))
            {
                ...
                //Save the mesh
                VertexPainterUtils.SaveMesh(curSelected);
            }
        }
                ...
                if (curEvent.type == EventType.KeyDown)
                {
                    ...
                    if (curEvent.keyCode == KeyCode.P)
                    {
                        if (m_CurrentState == VTXPainterState.None)
                        {
                            ...
                            m_CurrentState = VTXPainterState.Painting;
                        }
                        else if (m_CurrentState == VTXPainterState.Painting)
                        {
                            //Debug.Log("Setting None");
                            m_CurrentState = VTXPainterState.None;
                        }
                    }
                    ...
                    if (curEvent.keyCode == KeyCode.S)
                    {
                        if (m_CurrentState == VTXPainterState.None)
                        {
                            m_CurrentState = VTXPainterState.Sculpting;
                        }
                        else if (m_CurrentState == VTXPainterState.Sculpting)
                        {
                            m_CurrentState = VTXPainterState.None;
                        }
                    }
